...
#define TIMEOUT_FIRST_FRAME_USECS 100000 /* 100 ms in micro seconds */
...
static int stmvl53l1_stop(struct stmvl53l1_data *data)
{
	...
	/* fix ticket 1458514 deadlock multiple devices, cancel pending data collection */
...
static ssize_t stmvl53l1_show_unique_id(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct stmvl53l1_data *data = dev_get_drvdata(dev);
	uint64_t param;
...
static int ctrl_reg_access(struct stmvl53l1_data *data, void *p)
{
	...
	/* fix ticket #1466615 Race Condition (TOCTOU/Double Fetch) */
	total_byte = sizeof(struct stmvl53l1_register);
	...
	if (!reg.is_read) {
		...
		{ /* reduce the copy paylod to actual needed data(reg.cnt) */
			total_byte = offsetof(struct stmvl53l1_register,
				data.bytes[reg.cnt]);
		}
	}
				...
				struct list_head *head)
{
	...
	timeout = (2 * data->timing_budget);
	if ((data->wait_first_frame) && (timeout < TIMEOUT_FIRST_FRAME_USECS))
		timeout = TIMEOUT_FIRST_FRAME_USECS;
	mutex_unlock(&data->work_mutex);
...
static int ctrl_getdata_blocking(struct stmvl53l1_data *data, void __user *p)
{
	...
	if (data->meas.single_range_data.StreamCount == 0)
		data->wait_first_frame = false;
...
static int ctrl_roi(struct stmvl53l1_data *data, void __user *p)
{
	...
	rc = copy_from_user(&rois, p,
			sizeof(struct stmvl53l1_roi_full_t));
	if (rc) {
		rc = -EFAULT;
		goto done;
	}
...
static int ctrl_unique_id_data(struct stmvl53l1_data *data, void __user *p)
{
		...
		void __user *p)
{
	...
	case VL53L1_IOCTL_MZ_DATA_ADDITIONAL_BLOCKING:
		...
		break;
	case VL53L1_IOCTL_UID_DATA:
		vl53l1_dbgmsg("VL53L1_IOCTL_UID_DATA\n");
...
calibration_data.customer.algo__crosstalk_compensation_plane_offset_kcps);
	...
	if (mmeas->NumberOfObjectsFound > 1) {
		...
		vl53l1_dbgmsg("ABS_HAT2Y : 0x%X(%d:%d)\n",
				meas_array[1]->RangeMinMilliMeter << 16
				| meas_array[1]->RangeMaxMilliMeter,
...
int stmvl53l1_intr_handler(struct stmvl53l1_data *data)
{
	...
	/* fix ticket 1458514 deadlock multiple devices, defer the data collection */
	if ((data->enable_sensor) && (!data->poll_mode)) {
		rc = schedule_delayed_work(&data->dwork, 0);
	}
...
int stmvl53l1_setup(struct stmvl53l1_data *data)
{
	...
	data->is_delay_allowed = 1;
	rc = VL53L1_GetUID(&data->stdev, &dev_uid);
	data->is_delay_allowed = 0;
	if (rc) {
		vl53l1_errmsg("VL53L1_GetUID %d\n", rc);
		goto exit_unregister_dev_ps;
	}
	vl53l1_errmsg("device UID 0x%llx\n", dev_uid);
